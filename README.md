...
10)SMS Service : If Sadness in aggregated crosses 60% then SMS is sent to user's emergency number.
11)Medicine Aggregator : Given a medicine name, links of different e-commerce websites where the product is available for sale.
12)Nearby Psychiatrists : Using Location service from Browser, getting nearby psychiatrist clinics
13)Alexa Skill : Apart from web app, we have an Alexa Skill with same functionality which gives voice response as well as recommendations on alea app.
14)Chatbot for CBT(Cognitive Behavioral Test) : Chatbot powered by AI Services that improves over time as the patient interacts with it. It performs the role of expert in CBT Tests thus implanting positive thoughts in patient's mind.
